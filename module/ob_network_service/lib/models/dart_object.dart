    ...
    if (properties.isNotEmpty) {
      ...
      for (final DartProperty item in properties) {
        ...
        if (item is DartObject) {
          ...
          setString = ' ${item.name}:$lowName == null? []:$lowName';
          ...
          setString =
              DartHelper.setProperty(ok, item.name, item, this.className);
        }
        ...
        if (ok == 'is') {
          propertySb.writeLine(stringFormat(
              DartHelper.propertySOJB(item.propertyAccessorType),
              <String>[typeString, name, ok]));
